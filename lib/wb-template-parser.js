...
function parseChannel(item, parentGroup) {
    ...
    return {
        ...
        // isConfig: true если параметр определяет топологию каналов устройства.
        // Критерии: required:true (MAI6 sensorType) ИЛИ
        //           параметр без condition с enum и без адреса в диапазоне "тонкой настройки"
        isConfig:   !!(item.required || (
            !item.condition &&
            Array.isArray(item.enum) &&
            item.enum.length >= 2 &&
            item.group && !item.group.includes('press') && !item.group.includes('general')
        )),
    };
}
