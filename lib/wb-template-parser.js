...
function parseMeasurement(item, t) {
    ...
    if (base.regType === 'coil') return null;
...
function parseTemplate(source, filename = '') {
    ...
    for (const groupId of groupOrder) {
        ...
        // condition канала = condition первого measurement (в группе все одинаковые)
        const channelCondition = measurements[0]?.condition || null;
